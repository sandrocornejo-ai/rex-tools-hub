...
def extraer_fecha_proceso(nombre_archivo):
    """Extrae período del nombre del archivo → formato aaaa-mm.
    Soporta dos formatos:
      - _YYYY-MM al final:  "78706540-6_2026-07.csv", "jh_2026-07.csv"
      - _YYYYMM al final:   "787065406_202607.csv"
    """
    base = os.path.splitext(nombre_archivo)[0]
    # FIX 3: Detectar formato _YYYY-MM (con guión en la fecha)
    m = re.search(r'_(\d{4})-(\d{2})$', base)
    if m:
        return f"{m.group(1)}-{m.group(2)}"
    # Formato original _YYYYMM (sin guión)
    sufijo = base[-6:]
        ...
        if st.button("▶ Ejecutar validaciones"):
            ...
            for i, archivo in enumerate(archivos):
                ...
                if es_rexplus:
                    ...
                    # FIX 1: Normalizar nombre columna LRE.
                    # El CSV Previred/DT usa "Rut trabajador(1101)" sin espacio antes del paréntesis,
                    # pero el programa espera "Rut trabajador (1101)" con espacio.
                ...
                if es_rexplus and "Fecha de proceso" in df.columns:
                    ...
                    df["_fecha_proceso"] = fecha_proceso  # FIX 2: asignar al dataframe (no solo variable local)
